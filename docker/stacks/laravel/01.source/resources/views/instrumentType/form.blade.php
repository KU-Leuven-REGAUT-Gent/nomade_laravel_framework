@extends('template.form',['model'=>$instrumentType])
...
<div class="card mt-3">
    ...
    <div class="card-body">
        <select class="form-control" name="instrument_type_category_id">
            @foreach($categories as $category)
                <option {{ ( (old('instrument_type_category_id') ?? $instrumentType->category->id)==$category->id  ? 'selected="selected"' : '' ) }}
                    value="{{$category->id}}">{{$category->name_en}}</option>
            @endforeach
        </select>
        @error('instrument_type_category_id')<div style="color: red;">{{ $message }}</div>@enderror
    </div>
</div>
        ...
        <div class="row">
            <div class="col-md-9 ml-3">
                ...
                <input  type="checkbox"
                        ...
                        value='1'  {{  (old('parameters.'. $parameter->id.'.checked')?? $instrumentType->parameters->contains($parameter->id))=='1'  ? 'checked':''}} >
            ...
            <div class="col-md-2">
                <div class="input-group-prepend">
                    ...
                    <input  type="text"
                            ...
                            value="{{ old('parameters.'. $parameter->id . '.value') ?? ($instrumentType ->parameters->find($parameter->id) == null ? null : $instrumentType ->parameters->find($parameter->id)->pivot->value)}}">
